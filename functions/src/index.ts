...
app.post("/createMondayTask", upload.single("file"), async (req, res) => {
  try {
    ...
    if (file) {
      ...
      const fileUploadResponse = await axios.post("https://api.monday.com/v2/file", formDataFile, {
        headers: {
          Authorization: MONDAY_API_TOKEN,
